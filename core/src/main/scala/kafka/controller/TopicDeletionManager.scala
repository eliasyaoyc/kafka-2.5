...
class ControllerDeletionClient(controller: KafkaController, zkClient: KafkaZkClient) extends DeletionClient {
  ...
  // 删除 ZooKeeper 下待删除主题的标记节点 /admin/delete_topics下的给定topic子节点
  ...
  // 屏蔽主题分区数据变更监听器，具体实现原理其实就是取消/brokers/topics/节点数据变更的监听
 ...
 * @param controller
 */
class TopicDeletionManager(config: KafkaConfig, // KafkaConfig类，保存Broker端参数 delete.topic.enable 用于控制是否允许删除主题，默认是true
  ...
  /*
  主题因为某些事件可能一时无法完成删除，比如主题分区正在进行副本重分配等。一旦这些事件完成后，主题重新具备可删除的资格。
  此时，代码就需要调用 resumeDeletions 重启删除操作。
   */
  private def resumeDeletions(): Unit = {
    ...
    topicsQueuedForDeletion.foreach { topic =>
      ...
      // 如果该主题所有副本已经是ReplicaDeletionSuccessful状态即该主题已经被删除
